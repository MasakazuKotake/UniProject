...
namespace UniMath{
	...
	inline Vector2()
		...
		inline explicit Vector2( const float afCoordinate[2] ) : x( afCoordinate[0] ),
																y( afCoordinate[1] )
		{
		}
		...
		inline explicit Vector2( float* const r ) : x( r[0] ), y( r[1] )
		{
		}
		...
		*/
		inline HAL::Radian angleBetween(const HAL::Vector2& other) const
		{		
			...
			f =	HAL::Math::Clamp(f, (float)-1.0, (float)1.0);
			return HAL::Math::ACos(f);
		}
		...
		*/
		inline HAL::Radian angleTo(const HAL::Vector2& other) const
		{
			HAL::Radian angle = angleBetween(other);
		
			if (crossProduct(other)<0)			
				angle = (HAL::Radian)HAL::Math::TWO_PI - angle;		
